// forked from takumifukasawa's "[2015.8.22] プラグインを使わず3Dパーティクル" http://jsdo.it/takumifukasawa/S18u
var NUM = 100;
...
ctx.lineWidth = 0.5;
ctx.strokeStyle = "rgba(20, 180, 255, 0.5)";
...
var draw = function() {
	...
	for(var i=0; i<v.length; i++) {
        ...
        ctx.closePath();
        ctx.restore();
        dist[i] = { x: v[i].affineOut.x, y: v[i].affineOut.y};
    }
    ...
    for(var s=0; s<dist.length; s++) {
        for(var k=0; k<dist.length; k++) {
            ...
            var distan = distance(dist[s], dist[k]);
            

            if(distan < LIMIT) {
                ctx.save();
                ctx.beginPath();
                var alpha = (LIMIT - distan) / LIMIT;
                ctx.strokeStyle = "rgba(20, 180, 255, " + alpha + ")";
                ctx.moveTo(dist[s].x, dist[s].y);
