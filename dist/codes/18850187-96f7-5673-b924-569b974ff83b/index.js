...
(function($, win, doc) {
    ...
    Main.prototype.draw = function() {
        ...
        for(var s=0; s<this.dist.length; s++) {
            ...
            if(this.dist[s].rel < 0) continue;
            this.ctx.beginPath();   
            var alphaFill = this.dist[s].rel * 0.15;
            this.ctx.fillStyle = "rgba(20, 180, 255, " + alphaFill + ")";
            this.ctx.arc(this.dist[s].x, this.dist[s].y, this.dist[s].p*2.5, 0, Math.PI*2, false);
            this.ctx.fill();
            this.ctx.closePath();  
        }  
    };
