        ...
        for(var s=0, slen=this.lineObjects; s<slen; s++) {        
            for(var i=0, len=this.lines.length; i<len; i++) {
                ...
                for(var k=0, klen=position.array.length; k<klen; k++) {
                    var range = diff * k%5;
                    position.array[k*3] = Math.cos(diffTime*range*4)*distance*move*k;
                    position.array[k*3+1] = Math.sin(diffTime*range)*distance*move*k;
                    position.array[k*3+2] = Math.pow(Math.cos(diffTime*range),2)*distance*move*k;
                }
            ...
            this.lineGroups[s].rotation.x += 0.0012;
            this.lineGroups[s].rotation.y += 0.008;
            this.lineGroups[s].rotation.z += 0.006;
        }
