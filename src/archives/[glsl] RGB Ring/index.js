// forked from takumifukasawa's "[glsl] ring 角丸 パターン" http://jsdo.it/takumifukasawa/QPC2
// forked from takumifukasawa's "[glsl] ring 角丸" http://jsdo.it/takumifukasawa/Oj0z
// forked from takumifukasawa's "[glsl] rings" http://jsdo.it/takumifukasawa/qzDQ
// forked from takumifukasawa's "[glsl] plasma" http://jsdo.it/takumifukasawa/q4em
// forked from takumifukasawa's "WebGL API で GLSL を描く雛型" http://jsdo.it/takumifukasawa/Y3wh
'use strict';

var vertexShaderText = '\nattribute vec3 position;\nattribute vec2 uv;\nuniform mat4 modelViewProjectionMatrix;\nvarying vec2 vUv;\nvoid main(void) {\n    vUv = uv;\n    gl_Position = modelViewProjectionMatrix * vec4(position, 1.);\n}\n';

var fragmentShaderText = '\nprecision mediump float;\nvarying vec2 vUv;\nuniform float time;\n\nfloat circle(vec2 p, float r) {\n    return length(p) - r;\n}\n\nvec3 circleN(vec2 p, float r, float b, vec3 color) {\n    float d = circle(p, r);\n    float c = pow(1. - d, b);\n    return color * c;\n}\n\nvoid main(void) {\n    // -1 ~ 1\n    vec2 p = vUv * 2. - vec2(1.);\n\n    vec3 destColor = vec3(0.);\n    \n    destColor += circleN(\n        p + vec2(sin(time * 3.) * .1, cos(time * 1.) * .1),\n        .5 + sin(time * 1.) * .05,\n        32.,\n        vec3(1., 0., 0.)\n    );\n    destColor += circleN(\n        p + vec2(sin(time * 2.) * .1, cos(time * 2.) * .1),\n        .5 + sin(time * 1.5) * .05,\n        32.,\n        vec3(0., 1., 0.)\n    );\n    destColor += circleN(\n        p + vec2(sin(time * 1.) * .1, cos(time * 3.) * .1),\n        .5 + sin(time * 2.) * .05,\n        32.,\n        vec3(0., 0., 1.)\n    );\n    \n    gl_FragColor = vec4(destColor, 1.);\n}\n';
